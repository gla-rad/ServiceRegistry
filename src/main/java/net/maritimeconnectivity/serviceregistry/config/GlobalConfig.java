...
public class GlobalConfig {
    ...
    public ModelMapper modelMapper() {
        ...
        // ========================================================================== //
        // Create a map between the instances and the SECOM service instance object
        modelMapper.createTypeMap(Instance.class, ServiceInstanceObject.class)
                .implicitMappings()
                .addMappings(mapper -> {
                    mapper.using(ctx -> Optional.of(ctx)
                                    .map(MappingContext::getSource)
                                    ...
                                    .orElse(new String[]{}))
                            .map(Instance::getKeywords, ServiceInstanceObject::setKeywords);
                    mapper.using(ctx -> Optional.of(ctx)
                                    .map(MappingContext::getSource)
                                    .map(Xml.class::cast)
                                    .map(Xml::getContent)
